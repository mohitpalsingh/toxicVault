...
func (s *FileServer) handleMessageGetFile(from string, msg MessageGetFile) error {
	if !s.store.Has(msg.ID, msg.Key) {
		return fmt.Errorf("[%s] needs to serve this file (%s) but it doesn't exist on disk", s.Transport.Addr(), msg.Key)
	}
	...
	// First, sending the incomingStream byte to the peer, then the fileSize in int64
...
func (s *FileServer) handleMessageStoreFile(from string, msg MessageStoreFile) error {
	peer, ok := s.peers[from]
	if !ok {
		return fmt.Errorf("peer [%s] couldn't be found in the peer list", from)
	}
